...
export function CyberArena() {
  ...
  useFrame((state) => {
    ...
    if (elapsedTime <= nextSpawnTime.current) return;

    nextSpawnTime.current = elapsedTime + spawnInterval;

    const edge = Math.floor(Math.random() * 4);
    const coordinate = THREE.MathUtils.randFloat(
      -GAME_CONFIG.ARENA_BOUNDS + 3,
      GAME_CONFIG.ARENA_BOUNDS - 3,
    );
  ...
  return (
    <>
      ...
      <mesh position={[0, WALL_HEIGHT / 2, -MAP_SIZE / 2]}>
        ...
        <meshStandardMaterial color="#0b0d26" />
      </mesh>
      <mesh position={[0, WALL_HEIGHT / 2, MAP_SIZE / 2]}>
        <boxGeometry args={[MAP_SIZE, WALL_HEIGHT, WALL_THICKNESS]} />
        <meshStandardMaterial color="#0b0d26" />
      ...
      {MAP_OBSTACLES.map((building, index) => (
        <CyberBuilding key={index} building={building} index={index} />
      ))}
